...
In this lab we will analyze stream flow data from Tenderfoot Creek Experimental Forest (TCEF). TCEF is located in central Montana, north of White Sulphur Springs. [See here](https://www.fs.usda.gov/research/rmrs/forestsandranges/locations/tcef) for information about TCEF.  
...
df <- read_csv("PQdata.csv") # read in data
view(df)
str(df)
```
...
df %>% 
  ...
  geom_line(aes(y = BUB_Q, color = "BUB")) + 
...
### Daily Q plot
Plot the daily Q time series for the three watersheds with "Date" on the x axis and "Q" on the y axis. Use `color = ` to distinguish between the three watersheds. Also label the y-axis appropriatly as "Q (mm/day)". Feel free to to change other default settings to make the graph look nicer, too. Use `scale_x_date()` to place the ticks and labels every two years. We haven't looked at this yet, so you might have to do some googling.

```{r}
q_plot <- ggplot() +
  geom_line(data = PQ, aes(x = Date, y = Q, color = Watershed)) +
  labs(y = "Q (mm/day)") +
...
PQ_annual <- PQ %>%
  ...
  reframe(Qsum = sum(Q), Psum = sum(P), RR = Qsum/Psum) %>% 
  ungroup() %>% 
...
# create long form df for plotting with only quickflow and baseflow and the watershed names
HS_long <- HS_summary %>%
...
for (i in 1:3) {
  ...
  bf_filter <- 0.975 # set filter parameter
  temp <- baseflows(Qbf, bf_filter, n.reflected = 30, ts = "daily") # actual baseflow separation
...
```{r}
PQ_all <- right_join(HS_annual, PQ_annual) # combine the dfs
...
bfi_annual_plot <- ggplot(data = PQ_all, aes(x = RR, y = mean.bfi, color = Watershed, fill = Watershed)) + # color determines the line color, fill the confidence band color
  ...
  stat_smooth(method = "lm", alpha = 0.1) + # plot linear regression with confidence bands
...
```{r}
PQ_wide <- PQ_all %>% 
  select(-Qsum, -Psum) %>% # remove the columns with Q, P, and ET
  pivot_wider(names_from="Watershed", values_from="RR") # spread out the data for the watersheds and RRs

BUBcor <- cor.test(x = PQ_wide$BUB, y = PQ_wide$mean.bfi, method = "spearman")
BUBcor
LSCcor <- cor.test(x = PQ_wide$LSC, y = PQ_wide$mean.bfi, method = "spearman")
LSCcor
UTCcor <- cor.test(x = PQ_wide$UTC, y = PQ_wide$mean.bfi, method = "spearman")
UTCcor
```
